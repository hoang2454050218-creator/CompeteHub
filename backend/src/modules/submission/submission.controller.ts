...
export class SubmissionController {
  async submit(req: Request, res: Response, next: NextFunction) {
    try {
      if (!req.file) return next(new AppError('Vui lòng chọn tệp để tải lên', 400, 'MISSING_FILE'));
      ...
      sendSuccess(res, submission, 'Đã nhận bài nộp. Hệ thống đang chấm điểm.', 202);
    } catch (error) {
      next(error);
    }
  }
  ...
  async select(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      sendSuccess(res, submission, 'Đã chọn bài nộp');
    } catch (error) {
      next(error);
    }
  }
