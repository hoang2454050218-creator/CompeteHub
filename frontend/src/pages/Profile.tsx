...
export default function Profile() {
  ...
  if (isError) return <div className="max-w-4xl mx-auto px-4 py-20 text-center"><div className="card p-12"><h3 className="text-lg font-medium text-red-600 mb-2">Failed to load profile</h3><p className="text-gray-500">Please try again later.</p></div></div>;
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex flex-col sm:flex-row items-start gap-6">
          ...
          <div className="flex-1">
            ...
            <div className="flex flex-wrap gap-4 text-sm text-gray-500">
              ...
              <span className="flex items-center gap-1"><Trophy className="h-4 w-4" /> {profile._count?.enrollments || 0} competitions</span>
              <span className="flex items-center gap-1"><BarChart3 className="h-4 w-4" /> {profile._count?.submissions || 0} submissions</span>
              {profile.githubUrl && (
      ...
      {profile.bestResults?.length > 0 && (
        <div className="mt-8">
          <h2 className="text-xl font-bold text-gray-900 dark:text-white mb-4">Best Results</h2>
            ...
            {profile.bestResults.map((result: any) => (
              <div key={result.id} className="card p-4 flex items-center justify-between">
                <div>
                  ...
                  <p className="text-sm text-gray-500">Rank #{result.publicRank || '-'}</p>
                </div>
                <div className="text-right">
                  <div className="font-mono font-medium text-primary-600">{result.bestPublicScore?.toFixed(5)}</div>
                  <div className="text-xs text-gray-500">{result.submissionCount} submissions</div>
                </div>
              </div>
            ))}
          </div>
      ...
      {profile.competitionHistory?.length > 0 && (
        <div className="mt-8">
          <h2 className="text-xl font-bold text-gray-900 dark:text-white mb-4">Competition History</h2>
          <div className="space-y-3">
            {profile.competitionHistory.map((entry: any) => (
              <div key={entry.id} className="card p-4 flex items-center justify-between">
                <div>
                  ...
                  <p className="text-sm text-gray-500">{entry.competition.status} &middot; {entry.competition.evalMetric}</p>
                </div>
